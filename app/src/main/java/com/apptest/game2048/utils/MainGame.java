...
import com.apptest.game2048.backups.GameProgress;
import com.apptest.game2048.backups.SaveManager;
import com.apptest.game2048.models.Cell;
...
public class MainGame {
    ...
    public void newGame(boolean checkForBackUp) {
        if (grid == null) {
        ...
        if(checkForBackUp)
            SaveManager.getInstance().loadBackup(mView);
        else{
            score = 0;
            mView.elTime = 0;
        }
        ...
        mView.invalidate();
        // new code
        SaveManager.getInstance().saveBackup(mView);
       // Log.i("onStop","save");
    }
    ...
    private boolean isFirstTimePlay(){
        ...
        for (int i = 0; i < grid.field.length; i++) {
            if(!allCellEmpty) break;
            for (int j = 0; j < grid.field.length; j++) {
                if(grid.field[i][j]!=null){
                    allCellEmpty = false;
                    break;
                }
            }
